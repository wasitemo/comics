...
export default function PrivateBlogPage() {
  ...
  const fetchBlogs = async (pageNum: number) => {
    ...
    try {
      ...
      const response = await fetch(
        `https://comics-2mkb.onrender.com/protected/blog/get-blog?page=${pageNum}&limit=${limit}`,
        {
          method: "GET",
          headers: {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(
        `https://comics-2mkb.onrender.com/protected/blog/delete-blog/${id}`,
        {
          method: "DELETE",
          headers: {
            "Authorization": `Bearer ${token}`,
