...
export default function PrivateGenrePage() {
  ...
  const fetchGenres = async (pageNum: number) => {
    ...
    try {
      ...
      const response = await fetch(
        `https://comics-2mkb.onrender.com/protected/genre/get-genre?page=${pageNum}&limit=${limit}`,
        {
          method: "GET",
          headers: {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(
        `https://comics-2mkb.onrender.com/protected/genre/delete-genre/${id}`,
        {
          method: "DELETE",
          headers: {
            "Authorization": `Bearer ${token}`,
